...
p1 <- ggplot(plotting_data, aes(x = value, y = se2)) +
  ...
  labs(
    ...
    title = "Mean-Variance Relationship for Simulated Null Trait Data with Lognorm(-1.5,2.5) variances, sim 1"
  )   +
  theme_bw() +
...
#load("/hpc/group/baughlab/jb621/GEMMA_2_testing/sampledSNPs/preWhiten/fdr_analysis/FITmeanvar_null_results_052826.Rda")
...
hist(null_data$p_modified,
     main = "Null p-values for GEMMAmod: Var~LogNorm(-1.5,2.5)",
     xlab = "p-values",
     ylim = c(0,200))
...
ggplot(qq_df, aes(x = expected, y = observed, color = model)) +
  ...
  #   color = c("#2166ac")) + 
  scale_color_manual(
    values = c("GEMMA" = "#d6604d", "GEMMAmod" = "#2166ac" )
  ) +
  labs(
    #title    = "QQ-plot: GEMMA vs GEMMAmod (null simulations, Var~U(0,1.5))",
    title    = "QQ-plot: GEMMA vs GEMMAmod (null simulations, Fitted mean-variance)",
...
ggplot(data = power_results, aes(x = log10p_default, y = log10p_modified)) + geom_point() +
  labs(y = "GEMMAmod -log10(p-value)", x = "GEMMA -log10(p-value)", title = "Simulated Null Normally Distributed traits with homogenous variances and 1000 different SNPs") +
