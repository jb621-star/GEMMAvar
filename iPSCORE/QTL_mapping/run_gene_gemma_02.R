## ============================================================
## 02_run_gene_gemma.R
## Called by SLURM array — tests all SNPs for one gene
## Usage: Rscript 02_run_gene_gemma.R <gene_id>
## ============================================================
...
gene_id <- args[1]
#gene_id <- "ENSG00000051180.17"

if (is.na(gene_id) || gene_id == "") {
  stop("No gene_id supplied. Usage: Rscript 02_run_gene_gemma.R <gene_id>")
}

input_file  <- paste0("gemma_gene_inputs/", gene_id, ".rds")
output_file <- paste0("gemma_gene_outputs/", gene_id, ".rds")
...
cat(sprintf("Gene: %s | SNPs: %d\n", gene_id, ncol(inp$geno)))

# Run pygemma: all SNPs for this gene in one call
# Y: expression vector (n,)
# X: genotype matrix  (n x p) — pygemma tests each column as a separate SNP
# W: covariate matrix (n x q)
...
tryCatch({
  ...
  results$Element_ID <- gene_id
  
  saveRDS(results, output_file)
  cat("Done:", gene_id, "—", nrow(results), "SNPs tested\n")
  
}, error = function(e) {
  # Write an error marker so we can identify failed genes
  saveRDS(list(gene_id = gene_id, error = conditionMessage(e)),
          paste0("gemma_gene_outputs/ERROR_", gene_id, ".rds"))
  cat("ERROR for gene", gene_id, ":", conditionMessage(e), "\n")
  quit(status = 1)
})
