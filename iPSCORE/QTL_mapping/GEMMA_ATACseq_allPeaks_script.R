...
# 
# # Need a map of WGS_UUID to Subject_UUID to ATAC_UUID
...
save(atac_cov_ordered, file ="atac_cov_ordered.Rda")
load(file ="atac_cov_ordered.Rda")
#
idx <- match(meta_map_ordered$ATAC_UUID, colnames(edgeR_expr))
edgeR_atac_ordered <- edgeR_expr[, idx]
...
# ============================================================
# 2. BUILD PEAK GRANGES (± 100kb window)
...
peaks <- fread("ppc_atac_peak.bed")

# Do your expr IDs have version suffixes?
head(rownames(edgeR_atac_ordered))

# How many match as-is vs after stripping versions?
sum(rownames(edgeR_atac_ordered) %in% peaks$Element_ID)
...
gene_windows <- gene_annot_matched |>
  mutate(
    ...
    win_start = pmax(1, tss - 100000),   # 100 kb
    win_end   = tss + 100000
  ) |>
  with(GRanges(
    seqnames = chr,
