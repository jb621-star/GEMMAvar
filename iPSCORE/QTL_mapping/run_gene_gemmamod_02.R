## ============================================================
## 02_run_gene_gemmamod.R
## Called by SLURM array — runs GEMMAmod for all SNPs of one gene
## Usage: Rscript 02_run_gene_gemmamod.R <gene_id>
##
## Unlike standard GEMMA, voom must be re-run per SNP because the
## design matrix changes with each genotype vector. The loop over
## SNPs is therefore unavoidable, but parallelising at the gene
## level via SLURM provides the same overall speedup.
## ============================================================
...
gene_id <- args[1]
gene_id <- "ENSG00000183117.20"
#
if (is.na(gene_id) || gene_id == "") {
  stop("No gene_id supplied. Usage: Rscript run_gene_gemmamod_02.R <gene_id>")
}

input_file  <- paste0("gemma_gene_inputs/", gene_id, ".rds")
output_file <- paste0("gemmamod_gene_outputs/", gene_id, "NOSHRINKAGE.rds")

dir.create("gemmamod_gene_outputs", showWarnings = FALSE)

# Skip if already done (safe re-submission of failed jobs)
if (file.exists(output_file)) {
  cat("Output already exists, skipping:", gene_id, "\n")
  quit(status = 0)
}
...
# Load shared inputs
W      <- readRDS("gemma_gene_inputs/W.rds")
K      <- readRDS("gemma_gene_inputs/K.rds")
dge_lead <- readRDS("gemma_gene_inputs/dge_lead.rds")   # see note below

# Load gene-specific inputs
inp     <- readRDS(input_file)
geno    <- inp$geno      # samples x SNPs matrix
snp_ids <- inp$snp_ids
n_snps  <- length(snp_ids)

cat(sprintf("Gene: %s | SNPs: %d\n", gene_id, n_snps))
...
# ---- Per-SNP loop (voom requires per-SNP design matrix) ----
for (i in seq_len(n_snps)) {
  ...
  # Voom with this SNP's design matrix
  design_voom  <- model.matrix(~ causal_geno)
  y            <- voom(dge_lead, design = design_voom, plot = FALSE)
  
  # Extract expression and weights for this gene only
  expr_gene    <- y$E[gene_id, , drop = FALSE]           # 1 x n
  weights_gene <- y$weights[match(gene_id, rownames(dge_lead)), , drop = FALSE]  # 1 x n
  ...
  mod_results$Element_ID[i] <- gene_id
...
cat(sprintf("Done: %s — %d SNPs tested, %d errors\n",
            gene_id, n_snps, sum(is.na(mod_results$beta))))
